...
test("Diagnostic Endpoint Tests", async (t) => {
  ...
  function createAuthCookie() {
    ...
    setSession(mockRes, { id: "1", username: "admin", role: "Admin" });
    return mockRes.headers["Set-Cookie"].split(";")[0];
  }
  ...
  await t.test("Missing env vars", async () => {
    ...
    const req = { headers: { cookie: createAuthCookie() } };
    const res = mockRes();
  ...
  await t.test("Successful model list and strict isolation", async () => {
    ...
    global.fetch = async (url, options) => {
      ...
      if (href.includes("generativelanguage")) {
        assert.ok(href.includes("gemini-secret"), "Gemini key missing in URL");
        geminiCalled = true;
    ...
    // Strict OpenAI isolation assertions
    assert.strictEqual(openaiAuthHeaders.length, 2, "Exactly 2 OpenAI calls expected");
    assert.strictEqual(openaiAuthHeaders.filter(h => h === "Bearer openai-secret").length, 1, "Exactly 1 call with openai-secret");
    assert.strictEqual(openaiAuthHeaders.filter(h => h === "Bearer openai-image-secret").length, 1, "Exactly 1 call with openai-image-secret");
  ...
  await t.test("401 Unauthorized from provider", async () => {
    ...
    global.fetch = async () => ({
      ok: false, status: 401, json: async () => ({ error: { type: "invalid_request_error", code: "invalid_api_key" } })
    });
  ...
  await t.test("Explicit secret-leakage test", async () => {
    ...
    global.fetch = async () => ({
      ok: false, status: 400, json: async () => ({ error: { message: `Failed with key ${secret1}` } })
    });
