...
import { generateSeoArticle } from "../src/ai-providers.js";
...
// LinkedIn/Medium/Reddit/Facebook'a kopyala-yapıştır ile paylaşılacak
// SEO+GEO uyumlu bir yazı üretir. Airtable'a hiçbir şey yazmaz — kuyruktan
// tamamen bağımsız, anlık üret-göster-kopyala akışıdır (bkz. dashboard.html
// #seo-content bölümü).
export default async function handler(request, response) {
  ...
  try {
    ...
    const article = await generateSeoArticle(provider, product, body.topic, process.env);
    return response.status(200).json({ ok: true, article });
  } catch (error) {
    console.error(error);
